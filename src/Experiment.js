...
export const timesShared = comments => {
  return comments.map(c => c.timesShared).reduce((a, b) =>
  a + b, 0);
};

export const addFullName = comments => {
...
class Experiment extends Component {
  ...
  render() {
    ...
    */
    ...
    const verifiedComments = addFullName(
      getVerifiedComments(comments)
    );
    /* END TO-DO */
    ...
    return (
      <div className="experiment">
        ...
        <p className='shared'>Times Shared: {timesShared(verifiedComments)}</p>
      </div>
    );
  }
}
